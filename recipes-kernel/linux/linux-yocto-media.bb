...
SRC_URI = "git://linuxtv.org/media_tree.git;branch=${KBRANCH};name=machine;protocol=git \
           git://git.yoctoproject.org/yocto-kernel-cache;type=kmeta;name=meta;branch=master;destsuffix=${KMETA};protocol=https \
           file://0001-Change-kernel-boot-message.patch \
           file://v4l2-vivid.cfg \
           "
...
SRCREV_machine ?= '${@oe.utils.conditional("PREFERRED_PROVIDER_virtual/kernel", "linux-yocto-media", "${AUTOREV}", "b14257abe7057def6127f6fb2f14f9adc8acabdb", d)}'
SRCREV_meta ?= '${@oe.utils.conditional("PREFERRED_PROVIDER_virtual/kernel", "linux-yocto-media", "${AUTOREV}", "b14257abe7057def6127f6fb2f14f9adc8acabdb", d)}'
